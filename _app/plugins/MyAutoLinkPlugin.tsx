...
function extractMatchingNodes(
  ...
  unmodifiedBeforeNodes: FormattableTextNode[],
  matchingNodes: FormattableTextNode[],
  unmodifiedAfterNodes: FormattableTextNode[],
] {
  const unmodifiedBeforeNodes: FormattableTextNode[] = [];
  const matchingNodes: FormattableTextNode[] = [];
  const unmodifiedAfterNodes: FormattableTextNode[] = [];
...
): FormattableTextNode | undefined {
  const linkNode = $createAutoLinkNode(match.url, match.attributes);
  if (nodes.length === 1) {
    let remainingTextNode = nodes[0] as FormattableTextNode;
    let linkTextNode;
    if (startIndex === 0) {
      [linkTextNode, remainingTextNode] = remainingTextNode.splitText(endIndex) as [FormattableTextNode, FormattableTextNode];
    } else {
      [, linkTextNode, remainingTextNode] = remainingTextNode.splitText(
        startIndex,
        endIndex,
      ) as [FormattableTextNode, FormattableTextNode, FormattableTextNode];
    }
    const textNode = $createFormattableTextNode(match.text);
    ...
    for (let i = 1; i < nodes.length; i++) {
      ...
      if (currentNodeStart < endIndex) {
        if (currentNodeEnd <= endIndex) {
          ...
          remainingTextNode = endNode as FormattableTextNode;
        }
      }
    ...
    const selectedTextNode = selection
      ...
      : undefined;
    const textNode = $createFormattableTextNode(firstLinkTextNode.getTextContent());
...
function getTextNodesToMatch(textNode: FormattableTextNode): FormattableTextNode[] {
  // check if next siblings are simple text nodes till a node contains a space separator
  const textNodesToMatch = [textNode];
  let nextSibling = textNode.getNextSibling() as FormattableTextNode;
  while (
    nextSibling !== null &&
    ...
    nextSibling = nextSibling.getNextSibling() as FormattableTextNode;
  }
...
function useAutoLink(
  ...
  useEffect(() => {
    ...
    return mergeRegister(
      editor.registerNodeTransform(FormattableTextNode, (textNode: FormattableTextNode) => {

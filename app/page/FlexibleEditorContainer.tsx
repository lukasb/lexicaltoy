...
function FlexibleEditorLayout ({
  ...
  useBreakpoint(1536, isSmallWidthViewport, setIsSmallWidthViewport);

  if (isSmallWidthViewport) {
    return (
      <div className="grid grid-cols-1 gap-4">
        {openPageIds.map((pageId) => renderEditorContainer(pageId))}
      </div>
    );
  } else {
    return (
      <div className="container flex gap-4 w-full">
        <div className="column flex flex-col w-1/2">
          {openPageIds.filter((_, index) => index % 2 === 0).map((pageId) => renderEditorContainer(pageId))}
        </div>
        <div className="column flex flex-col w-1/2">
          {openPageIds.filter((_, index) => index % 2 !== 0).map((pageId) => renderEditorContainer(pageId))}
        </div>
  ...
  function renderEditorContainer(pageId: string) {
